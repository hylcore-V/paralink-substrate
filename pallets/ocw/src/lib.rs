...
pub const HTTP_REMOTE_REQUEST: &str = "http://127.0.0.1:3030/test";
...
decl_storage! {
	trait Store for Module<T: Trait> as OcwRuntime {
...
decl_error! {
	pub enum Error for Module<T: Trait> {
		...
		// Error returned when fetching results
		HttpFetchingError,
...
decl_module! {
	pub struct Module<T: Trait> for enum Call where origin: T::Origin {
		...
		fn offchain_worker(block_number: T::BlockNumber) {
			...
			// 2. Fetching JSON via http requests in ocw
...
impl<T: Trait> Module<T> {
	...
	fn http_test_stub() -> Result<(), Error<T>> {
		// Create a reference to Local Storage value.
		// Since the local storage is common for all offchain workers, it's a good practice
		// to prepend our entry with the pallet name.
		let s_info = StorageValueRef::persistent(b"offchain-demo::response-stub");
		...
		if let Some(Some(response)) = s_info.get::<NodeResponse>() {
			// gh-info has already been fetched. Return early.
			debug::info!("cached response-stub: {:?}", response);
			return Ok(());
		}
		...
		let mut lock = StorageLock::<BlockAndTime<Self>>::with_block_and_time_deadline(
			b"ocw::lock", LOCK_BLOCK_EXPIRATION,
		...
		if let Ok(_guard) = lock.try_lock() {
			match Self::fetch_n_parse() {
				Ok(response) => { s_info.set(&response); }
	...
	fn fetch_n_parse() -> Result<NodeResponse, Error<T>> {
		...
		let gh_info: NodeResponse =
			serde_json::from_str(&resp_str).map_err(|_| <Error<T>>::ResponseParsingError)?;
		Ok(gh_info)
	}
